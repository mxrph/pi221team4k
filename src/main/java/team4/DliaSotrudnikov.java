package team4;
	...
	public class DliaSotrudnikov extends Uslovie {
		...
		public float calculation(int summa_try, int mesyac_try, int dosr_try) {
			...
			float procRaznica; // Объявление переменной, которая рассчитывает разницу
		    ...
		    dosr(dosr_try);
		       this.mproc = proc/1200; // Вычисление ежемесячного процента
		       this.mesyac = (float) (summa_try*(mproc+(mproc/(((Math.pow(1 + mproc, mesyac_try)))-1)))); // Формула вычисления аннуитетного платежа
		       this.pereplata = mesyac * mesyac_try - summa_try; // Вычисление переплаты
